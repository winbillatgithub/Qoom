...
void CXTabCtrl::SetInfoColorFlag(bool bInfo, int iIndex, COLORREF cr) 
{
	m_crInfo = cr;
	m_arrayInfoTab[iIndex] = bInfo;
	// Redraw it.
	CRect rect;	
...
void CXTabCtrl::AddTab(LPTSTR lpszCaption, int iImage)
{
	TCITEM item;
	...
	m_arrayInfoTab.Add(FALSE);
}

BOOL CXTabCtrl::InsertXItem(int nItem, LPCTSTR lpszItem, int nImage)
{
	TCHAR szText[64];
	swprintf(szText, lpszItem); 
...
void CXTabCtrl::DeleteTab(int iIndex)
{
	...
	m_arrayInfoTab.RemoveAt(iIndex);
...
{	
	...
	BOOL bInfo = m_arrayInfoTab[nTabIndex];
	...
	COLORREF crInfo = bInfo ? m_crInfo : GetSysColor(COLOR_BTNTEXT);
	...
	else
	{
		//** selected item -----
		if (bInfo) {
			pDC->SetTextColor(crInfo);
		} else if (bSelected) {
			pDC->SetTextColor(crSelected);
		} else if (m_bColorMouseOver && nTabIndex == m_iIndexMouseOver) {
...
BOOL CXTabCtrl::SelectTabByName(LPCTSTR lpszTabCaption)
{
	...
	for (int i=0; i < GetItemCount(); i++)
	{
		...
		if (lstrcmp(lpszTabCaption, item.pszText) == 0)
			return SelectTab(i);
	}
