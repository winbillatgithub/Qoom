...
//return BYTEs, 
//dwSize in WCHAR
LPSTR _lW2A(LPWSTR szUnicode, DWORD dwSize, DWORD& dwRetSize)
{
...
#else
				DWORD dwRet;
                result = (HINSTANCE) WinExec(_lW2A(key, ::lstrlen(key), dwRet),showcmd);
#endif
            }
        }
	}
...
// Activate the link
void CHyperLink::FollowLink(BOOL bClick) 
{
	if (bClick == FALSE) {
		...
		HINSTANCE result = ShellExecute(NULL, _T("open"), 
			_T("mailto:michael_shur@hotmail.com?subject=Advices for Qoom1.0..."),NULL,NULL, SW_SHOWNORMAL);
